...
function provideBuilder() {
  return class PythonProvider extends events__WEBPACK_IMPORTED_MODULE_0__["EventEmitter"] {
    constructor(cwd) {
      super();
      this.cwd = cwd;
    ...
    async isEligible() {
      ...
      const pythonVersion = atom.config.get(_package_json__WEBPACK_IMPORTED_MODULE_4__.name + '.pythonVersion');
    ...
    settings() {
      const errorMatch = ['\\s+File \\"(?<file>.*)\\", line (?<line>\\d+).*?\\n(?:[\\s|\\S]+)\\n(?<message>.*rror:.*)']; // User settings

      const customArguments = atom.config.get(_package_json__WEBPACK_IMPORTED_MODULE_4__.name + '.customArguments').trim().split(' ');
      customArguments.push('{FILE_ACTIVE}');
      const customInterpreter = atom.config.get(_package_json__WEBPACK_IMPORTED_MODULE_4__.name + '.customInterpreter');
      const pythonVersion = atom.config.get(_package_json__WEBPACK_IMPORTED_MODULE_4__.name + '.pythonVersion');
...
module.exports = {"name":"build-python","version":"0.9.3","description":"Atom Build provider for python/python3/py, compiles Python","repository":"https://github.com/idleberg/atom-build-python","license":"MIT","scripts":{"build":"webpack --mode none","dev":"webpack --mode none --watch","lint":"eslint ./src","test":"npm run lint"},"keywords":["buildprovider","compile","python","python3","linter","lint"],"main":"lib/provider.js","engines":{"atom":">=1.0.0 <2.0.0"},"providedServices":{"builder":{"description":"Compiles Python","versions":{"2.0.0":"provideBuilder"}}},"package-deps":["build"],"dependencies":{"atom-package-deps":"^5.1.0"},"devDependencies":{"@babel/core":"^7.4.5","babel-eslint":"^10.0.2","babel-loader":"^8.0.6","eslint":"^6.0.0","eslint-config-atom-build":"^4.0.0","husky":"^3.0.0","webpack":"^4.35.0","webpack-cli":"^3.3.5"},"husky":{"hooks":{"pre-commit":"npm run lint"}}};
